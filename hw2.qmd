...
tm_shape(holc) + 
  ...
  tm_title(text = "LA County Home Owners' Loan Corporation HOLC Grade") + 
  tm_basemap() +
  tm_scalebar() + 
  tm_compass() 
```
...
ej_holc_per_table <- knitr::kable(
  ...
  caption = "Los Angeles Census Percent of Group by HOLC Grade",
  align = "c"
)
ej_holc_per_table 
```
...
holc_low_inc %>% 
  ggplot(aes(x = grade, y = inc_avg, fill = grade)) +
  geom_col() +
...
holc_pm %>% 
  ggplot(aes(x = grade, y = pm_avg, fill = grade)) +
  geom_col() +
  scale_fill_manual(values = c(
    "A" = "darkgreen",
    "B" = "lightblue",
...
ggplot(bird_obs_grade, aes(x = "Bird Observations",
  ...
  labs(
    title = "Proportion of Bird Observations Across HOLC Graded Neighborhoods in Los Angeles County",
